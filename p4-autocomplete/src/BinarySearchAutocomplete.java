...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		PriorityQueue<Term> kTerms = new PriorityQueue<>(last - first + 1, Comparator.comparing(Term::getWeight));
		...
		for(int i = first; i <= last; i++) {
			...
			if(kTerms.size() < k) {
				...
				if(kTerms.peek().getWeight() < term.getWeight()) {
				kTerms.remove();
				kTerms.add(term);
			}
		}

		// using linked list so I can add to the beginning of
		// the list since kTerms.remove() gives us values in reverse
		// order of weight.
		LinkedList<Term> list1 = new LinkedList<>();
		int size = kTerms.size();
		for(int i = 0; i < size; i++) {
			list1.addFirst(kTerms.remove());
		}

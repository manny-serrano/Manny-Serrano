...
public class HuffProcessor {
	...
	private int [] getCounts(BitInputStream in){
		...
		while(true){
			int chonk = in.readBits(BITS_PER_WORD);
			if(chonk ==-1){
				break;
			}
			counts[chonk]++; 
	...
	private HuffNode makeTree(int[] counts){
		PriorityQueue<HuffNode> priority = new PriorityQueue<>();
		for (int i =0; i<counts.length; i++){
			if(counts[i]>0){
				priority.add(new HuffNode(i, counts[i], null, null)); 

			}

		}
		priority.add(new HuffNode(PSEUDO_EOF, 1, null, null)); 
		while(priority.size()>1){
			HuffNode left = priority.remove();
			HuffNode right = priority.remove();
			HuffNode newt = new HuffNode(left.weight+right.weight, left.weight+right.weight, left, right);
			priority.add(newt); 
		}

		HuffNode root = priority.remove();
		return root; 
	}
	...
	public void decompress(BitInputStream in, BitOutputStream out){
		...
		HuffNode treeroot = readTree(in); 
		HuffNode current = treeroot; 
	...
	private HuffNode readTree(BitInputStream in){
		int bit = in.readBits(1);
		if(bit == -1) {
			throw new HuffException("reading bits failed lil bro");
		}
		if(bit== 0){
